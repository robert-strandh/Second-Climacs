...
(defun delete-node (node)
  (let ((analyzer (analyzer node)))
    (splay node)
    (cond ((null (clump-binary-tree:left node))
	   (setf (contents analyzer) (clump-binary-tree:right node)))
	  ((null (clump-binary-tree:right node))
	   (setf (contents analyzer) (clump-binary-tree:left node)))
	  (t
	   (let ((prev (loop for n = (clump-binary-tree:left node)
			       then (clump-binary-tree:right n)
			     until (null (clump-binary-tree:right n))
			     finally (return n))))
	     (splay prev)
	     (let ((next (clump-binary-tree:right node)))
    ...
    (flet ((create (buffer-line)
	     (let ((new (make-instance 'node
		   ...
		   (let ((prev (find-node (1- current-line-number) analyzer)))
		     (splay prev)
		     (let ((next (clump-binary-tree:right prev)))
		       (setf (clump-binary-tree:right prev) nil)
		       (setf (clump-binary-tree:left new) prev)
		       (setf (clump-binary-tree:right new) next)
		       (setf (contents analyzer) new)))))
