...
(defun warn-unused-numeric-parameter (parameter)
...
(defclass comment-entry (entry)
  ())

(defun semicolon (stream char)
  (declare (ignore char))
  (change-class *parent-entry* 'comment-entry)
...
(defun single-quote (stream char)
  ...
  (let ((*backquote-in-subforms-allowed-p* *backquote-allowed-p*))
    (list 'quote (custom-read stream t nil t))))
...
(defclass string-entry (entry)
  ())

(defun double-quote (stream char)
  (change-class *parent-entry* 'string-entry)
...
(defun backquote (stream char)
  ...
  (let ((*backquote-depth* (1+ *backquote-depth*))
    ...
    `(quasiquote ,(custom-read stream t nil t))))
...
(defun comma (stream char)
  ...
  (let* ((char2 (read-char stream t nil t))
	 ...
	 (*backquote-in-subforms-allowed-p* *backquote-allowed-p*))
    (let ((form (custom-read stream t nil t)))
      (if at-sign-p
...
(defun left-parenthesis (stream char)
  ...
  (let ((reversed-result '())
	...
	(*backquote-in-subforms-allowed-p* *backquote-allowed-p*))
    (catch 'end-of-list
      (loop for object = (custom-read stream t nil t)
	    do (if (eq object *consing-dot*)
		   (progn (setf *consing-dot-allowed-p* nil)
			  (catch 'end-of-list
			    (setf tail (custom-read stream t nil t))
			    ;; If we come here, we got an expression
			    ;; following the consing dot.
			    (catch 'end-of-list
			      (custom-read stream t nil t)
			      ;; If we come here, we got more than
			      ;; one expression following the
			      ;; consing dot.
			      (error 'multiple-objects-following-consing-dot
				     :stream stream))
			    ;; If we come here, we have a consing dot
			    ;; followed by one expression followed by
			    ;; a right parenthesis, which is correct.
			    (return-from left-parenthesis
			      (nreconc reversed-result tail))))
		   (push object reversed-result))))
    ;; If we come here, we got a right parenthesis without having seen
    ;; a consing dot.
    (return-from left-parenthesis
      (nreverse reversed-result))))

(defclass unbalanced-right-parenthesis ()
  ())
...
(defun right-parenthesis (stream char)
  ...
  (handler-case
    ...
    (control-error ()
      (make-instance 'unbalanced-right-parenthesis))))
...
(defun sharpsign-single-quote (stream char parameter)
  ...
  (unless (null parameter)
    (warn-unused-numeric-parameter parameter))
  (let ((*backquote-in-subforms-allowed-p* *backquote-allowed-p*))
    `(function ,(custom-read stream t nil t))))
...
(defun sharpsign-left-parenthesis (stream char parameter)
  ...
  (let ((*backquote-in-subforms-allowed-p* *backquote-allowed-p*))
	...
	(let ((reversed-elements '()))
	  ...
	  ;; Come here when we got a right parenthesis
	  ...
	  (catch 'end-of-list
	    (progn 
	      (loop until (= index parameter)
		    for object = (custom-read stream t nil t)
		    do (setf (aref result index) object)
		       (incf index))
	      ;; If we come here, we found as many expressions as the
	      ;; paremeter indicates.  There are now two
	      ;; possibilities.  If we attempt to read another
	      ;; expression, and it throws end-of-list, then we saw a
	      ;; right parenthesis.  On the other hand, if we attempt
	      ;; to read another expression and nothing was thrown,
	      ;; then we have more expresions than the parameter indicates. 
	      (catch 'end-of-list
		;; Read until we get a throw.
		(loop do (custom-read stream t nil t)))
	      ;; Come here when we saw a right parenthesis.
	      (return-from sharpsign-left-parenthesis result)))
	  ;; Come here if we saw a right parenthesis before having read
	  ;; enough expressions to fill the result.
	  (if (zerop index)
	      ;; No objects were supplied, but the parameter given
	      ;; was greater than zero.  Fill with NIL.
	      (fill result nil)
	      ;; Duplicate the last object supplied.
	      (fill result (aref result (1- index)) :start index))
	  (return-from sharpsign-left-parenthesis result)))))
...
(defun sharpsign-dot (stream char parameter)
  ...
  (unless (null parameter)
    (warn-unused-numeric-parameter parameter))
  (eval (custom-read stream t nil t)))
...
(defun determine-dimensions (rank initial-contents)
  (cond ((zerop rank)
	...
	((not (or (and (listp initial-contents)
		       (proper-list-p initial-contents))
...
(defun sharpsign-a (stream char parameter)
  (declare (ignore char))
  (let ((init (custom-read stream t nil t)))
...
(defun sharpsign-c (stream char parameter)
  ...
  (unless (null parameter)
    (warn-unused-numeric-parameter parameter))
  (apply #'complex (custom-read stream t nil t)))
...
(defun sharpsign-p (stream char parameter)
  ...
  (unless (null parameter)
    (warn-unused-numeric-parameter parameter))
  (let ((expression (custom-read stream t nil t)))
...
(defun check-feature-expression (feature-expression)
  (unless (or (symbolp feature-expression)
	      (and (proper-list-p feature-expression)
...
(defun sharpsign-plus (stream char parameter)
  ...
  (let* ((*package* (find-package '#:keyword))
	 (feature-expression (custom-read stream t nil t)))
