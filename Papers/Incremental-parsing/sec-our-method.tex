...
\item The \emph{edit protocol} is used whenever items are inserted or
  deleted from the buffer, and when client code needs to determine
  what item is currently in a particular location in the buffer.
  An edit operation is typically invoked as a result of a keystroke,
  but an arbitrary number of edit operations can happen as a result of
  a keystroke, for example when a region is inserted or deleted, or
  when a keyboard macro is executed.
