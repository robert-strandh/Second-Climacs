...
location $L$ may be moved.  It suffices to pop an element off of one
...
\begin{itemize}
\item If the parse result is the first top-level parse result of the
  buffer, then the value of this slot is the absolute line number of
  the initial character of this parse result.  The first line of the
  buffer is numbered $0$.
\item If the parse result is a top-level parse result other than the
  first of the buffer, then the value of this slot is the number of
  lines between the value of the slot \texttt{end-line} of the
...
If the value of \texttt{start-line} is greater than $0$, then the
value of the slot \texttt{start-column} is the absolute column number
of the initial character of this parse result.  A value of $0$ means
the leftmost column.

If the value of \texttt{start-line} is equal to $0$, then the value of
the slot \texttt{start-column} is always strictly greater than $0$,
and it is computed as follows:

\begin{itemize}
\item If the parse result is the first top-level parse result of the
  buffer, then the value of this slot is the absolute column number of
  the initial character of this parse result.
\item If the parse result is a top-level parse result other than the
  first of the buffer, then the value of this slot is the number of
  columns between the value of the slot \texttt{end-column} of the
  preceding parse result and the initial character of this parse
  result.
\item If this parse result is the first in a list of children of some
  parent parse result, then the value of this slot is the number of
  columns between the value of the slot \texttt{start-column} of the parent
  parse result and the initial character of this parse result.
\item If this parse result is the child other than the first in a list
  of children of some parent parse result, then the value of this slot
  is the number of columns between the value of the slot
  \texttt{end-column} of the preceding sibling parse result and the
  initial character of this parse result.
\end{itemize}
