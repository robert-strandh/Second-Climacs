...
This phase consists of repeated applying a custom version of the \cl{}
\texttt{read} function to the contents of the buffer in order to
obtain a \emph{succession of top-level expressions}.  
...
\item Invalid tokens.  An invalid token is either a token that has an
  ...
  even followed by a newline) or an odd number of \emph{multiple
    escape} characters.
\item A left parenthesis at the top level of the buffer.
