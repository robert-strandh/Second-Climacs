...
(flet ((draw-rectangles (context
  ...
  (defmethod draw-token-wad :before
    ...
    (declare (ignore cache))
    (let ((package-marker (ip:package-marker-1 token)))
      (unless (or (null package-marker)
                  (zerop package-marker))
        (let* ((start  (ip:start-column wad))
               (mid    (+ start package-marker 1))
               (end    (ip:end-column wad))
               (height (ip:height wad)))
          (unless (not (zerop height))
            (draw-rectangles context start-ref start mid end
                             clim:+pink+ clim:+red+))))))
  ...
  (defmethod draw-token-wad :before
    ...
    (declare (ignore cache))
    (let ((package-marker (ip:package-marker-1 token)))
      (unless (null package-marker)
        (let* ((start  (ip:start-column wad))
               (mid    (+ start package-marker))
               (end    (ip:end-column wad))
               (height (ip:height wad)))
          (unless (not (zerop height))
            (draw-rectangles context start-ref start mid end
